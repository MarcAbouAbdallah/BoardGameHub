...
public class PersonalCollectionService {
    ...
    /*
     * New method to check if a particular game copy is available by checking the
     * current date against accepted borrow requests
     */
    public boolean isAvailable(GameCopy copy) {
        ...
        for (BorrowRequest b : list) {
            if (b.getStatus() != BorrowStatus.ACCEPTED)
                continue;
            if (b.getStartDate().equals(now) || b.getEndDate().equals(now)
                    || (b.getStartDate().before(now) && b.getEndDate().after(now))) {
                return false;
            }
        }
        return true;
